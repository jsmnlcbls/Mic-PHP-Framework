...
use Mic\Representations\FileDownload;

use Mic\Library\DefaultResponse;

class Download extends DefaultResponse
{	
	public function output()
	{
		$representation = $this->getRepresentation();
		if ($representation instanceof FileDownload) {
			$filename = $this->getRepresentation()->getFilename();
			$header = "attachment; filename={$filename}";
			$this->setHeader(self::HEADER_CONTENT_DISPOSITION, $header);
		}
		parent::output();
	}
}